...
`tools/list` at `https://gateway.pipeworx.io/vat/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/validate_vat \
  -H 'Content-Type: application/json' \
  -d '{"vat":"DE123456789"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/validate_vat`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
